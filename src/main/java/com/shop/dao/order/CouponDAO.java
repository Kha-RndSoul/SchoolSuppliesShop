...
public class CouponDAO extends BaseDao {
    ...
    static {
        ...
        data.put(1, new Coupon(
                1, "WELCOME10", "PERCENT",
                new BigDecimal("10"), new BigDecimal("0"),
                100, 0, now, null, true, now
        ));
        data.put(2, new Coupon(
                2, "SUMMER50K", "FIXED",
                new BigDecimal("50000"), new BigDecimal("200000"),
                50, 0, now, null, true, now
        ));
        data.put(3, new Coupon(
                3, "FREESHIP", "FIXED",
                new BigDecimal("30000"), new BigDecimal("100000"),
                200, 0, now, null, true, now
        ));
    }
    ...
    public boolean existsByCode(String code) {
        return get().withHandle(h ->
                h.createQuery(
                                "SELECT COUNT(id) > 0 FROM coupons WHERE code = :code"
                        )
                        .bind("code", code)
                        .mapTo(Boolean.class)
                        .one()
    ...
    public boolean isValidCoupon(String code) {
        return get().withHandle(h ->
                h.createQuery(
                                "SELECT COUNT(id) > 0 FROM coupons " +
                                        "WHERE code = :code " +
                                        "AND is_active = true " +
        ...
        get().useHandle(h -> {
            PreparedBatch batch = h.prepareBatch(
                    "INSERT INTO coupons " +
                            ...
                            "VALUES (:id, :code, :description, :discountType, :discountValue, " +
                            ":minOrderAmount, :maxDiscountAmount, :maxUses, 0, " +
                            ":startDate, :expiryDate, :isActive, NOW())"
            );
        ...
        get().useHandle(h -> {
            h.createUpdate(
                            "UPDATE coupons SET " +
                                    ...
                                    "max_uses = :maxUses, start_date = :startDate, " +
                                    "expiry_date = :expiryDate, is_active = :isActive, " +
