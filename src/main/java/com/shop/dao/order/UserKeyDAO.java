...
public class UserKeyDAO extends BaseDao {
    ...
    public UserKey getActiveByCustomerId(int customerId) {
        return get().withHandle(h ->
                h.createQuery(
                                "SELECT id, customer_id, public_key, is_active, source, " +
    ...
    // Lấy key theo id (để verify đơn hàng cũ)
    ...
    // Lấy toàn bộ lịch sử key của customer
    ...
    // Deactivate tất cả key cũ của customer (khi tạo key mới)
    // Key cũ vẫn còn trong DB, chỉ set is_active = false
    public void deactivateAllByCustomerId(int customerId) {
        get().useHandle(h ->
                h.createUpdate(
                                "UPDATE user_keys SET is_active = false " +
                                        "WHERE customer_id = :customerId AND is_active = true"
                        )
                        .bind("customerId", customerId)
                        .execute()
    ...
    // Báo mất khóa: ghi thời điểm báo mất + deactivate
