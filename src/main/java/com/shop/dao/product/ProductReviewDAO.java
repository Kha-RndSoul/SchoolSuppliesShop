...
public class ProductReviewDAO extends BaseDao {
    ...
    // Lấy đánh giá sản phẩm đã duyệt theo productId + TÊN KHÁCH HÀNG
    public List<ProductReview> getByProductId(int productId) {
        return get().withHandle(h ->
                h.createQuery(
                                "SELECT pr.id, pr.product_id, pr.customer_id, pr.rating, pr.comment, " +
                                        "pr.status, pr.created_at, c.full_name as customerName " +
    ...
    // Lấy TẤT CẢ đánh giá theo productId (cả chưa duyệt) - Dùng cho Admin
    ...
    public int insertReview(ProductReview review) {
        ...
        return get().withHandle(handle ->
                handle.createUpdate("INSERT INTO product_reviews (product_id, customer_id, rating, comment, created_at, status) " +
                                "VALUES (:productId, :customerId, :rating, :comment, NOW(), :status)")
                        .bind("productId", review.getProductId())
                        .bind("customerId", review.getCustomerId())
                        .bind("rating", review.getRating())
                        .bind("comment", review.getComment())
                        .bind("status", 1) // (0 chờ duyệt ,1 đã duyệt)
                        .execute()
        );
    }
